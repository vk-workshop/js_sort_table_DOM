...
function prepareLetters(rows, pos) {
    rows.sort((a, b)=>{
        return a.children[pos].textContent.localeCompare(b.children[pos].textContent);
    });
}
    ...
    th.addEventListener("click", (e)=>{
        ...
        if (e.target.textContent === "Salary") {
            const sortedRows = tableRows.sort((a, b)=>{
                const salaryOne = a.children[3].textContent.slice(1, -1).replace(",", "");
                const salaryTwo = b.children[3].textContent.slice(1, -1).replace(",", "");
